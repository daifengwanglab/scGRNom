scGRNom_disGenes <- function(df, gwas_snps, extension_bps){
  ...
  tfbs_setup_results_mg_en = tfbs_setup_en(df, gwas_snps,extension_bps)
  tfbs_setup_results_mg_pr = tfbs_setup_pr(df, gwas_snps,extension_bps)
  
  if(is.null(tfbs_setup_results_mg_en)&is.null(tfbs_setup_results_mg_pr)){
    disease_genes = data.frame(disease_genes = NULL)
    return(disease_genes)
  }
  
  if(!is.null(tfbs_setup_results_mg_en)){
    mg_en_mb = tfbs_interruption(tfbs_setup_results_mg_en)
    mg_broken_en_tfbs = ct_tfbs_interruption(df, mg_en_mb)
    en_disease_genes = unique(as.character(mg_broken_en_tfbs$TG))
  }
  
  if(!is.null(tfbs_setup_results_mg_pr)){
    mg_pr_mb = tfbs_interruption(tfbs_setup_results_mg_pr)
    mg_broken_pr_tfbs = ct_tfbs_interruption(df, mg_pr_mb)
    pr_disease_genes = unique(as.character(mg_broken_pr_tfbs$TG))
  }
  
  if(exists('en_disease_genes')&exists('pr_disease_genes')){
    disease_genes = data.frame(unique(c(as.character(en_disease_genes),as.character(pr_disease_genes))))
    colnames(disease_genes) = 'disease_genes'
  }else if(exists('en_disease_genes')&!exists('pr_disease_genes')){
    disease_genes = data.frame(unique(as.character(en_disease_genes)))
    colnames(disease_genes) = 'disease_genes'
  }else if(!exists('en_disease_genes')&exists('pr_disease_genes')){
    disease_genes = data.frame(unique(as.character(pr_disease_genes)))
    colnames(disease_genes) = 'disease_genes'
  }
  return(disease_genes)
}
...
tfbs_setup_en = function(ct_grn, gwas,extension) {
  ...
  if(length(gwas_index[[3]])!=0){
    # Get SNPs from RSID for enhancer
...
# tfbs_setup:
#   - Sets up overlapping GRanges for SNPs between disease GWAS and GRN
tfbs_setup_pr = function(ct_grn, gwas,extension) {
...
ct_tfbs_interruption = function(ct_grn, mb_results) {
  # Perform setup
  curr_grn = grn_setup(ct_grn)
  # Create GRanges objects for (en)hancer and (pr)omoter
  curr_en_gr = GRanges(seqnames = curr_grn$en_chrs,
  ...
  mb_results_df = data.frame(mb_results)
  mb_results_df$chrs = as.numeric(gsub("[^0-9.-]", "", mb_results_df$seqnames))
  #mb_results_en_df = mb_results_en_df[which(mb_results_en_df$effect == 'strong'),]
  
  mb_results_gr = GRanges(
    seqnames = mb_results_df$chrs,
    IRanges(start = mb_results_df$start,
            end = mb_results_df$end)
  )
  
  grn_mb_overlap = as.data.frame(
    findOverlaps(
      query = curr_en_gr,
      subject = mb_results_gr,
      type = 'any',
      select = "all",
      ignore.strand = T
    )
  ...
  grn_mb_overlap_df = unique(curr_grn[unlist(unique(grn_mb_overlap)), ])
  
  return(grn_mb_overlap_df)
}
